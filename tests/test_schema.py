...
@pytest.mark.parametrize("fmt", ["acdc/v1.0.0", "acdc/v1.1.0"])
...
def test_ecr_schema_with_good(fmt, schema, sample):
    with open(f'{Path(__file__).parent}/../schema/{fmt}/{schema}', 'r') as schma, \
            open(f'{Path(__file__).parent}/../samples/{fmt}/{sample}', 'r') as smple:
...
def test_legal_entity_chain():
    qvi = json.load(open(f'{__path()}/../schema/acdc/v1.1.0/qualified-vLEI-issuer-vLEI-credential.json', 'r'))
    le = json.load(open(f'{__path()}/../schema/acdc/v1.1.0/legal-entity-vLEI-credential.json', 'r'))

    assert le['properties']['e']['oneOf'][1]['properties']['qvi']["properties"]['s']['oneOf'][1]['const'] == qvi[coring.Saids.dollar]


def test_ecr_auth_chain():
    auth = json.load(open(f'{__path()}/../schema/acdc/v1.1.0/ecr-authorization-vlei-credential.json', 'r'))

    assert auth['properties']['e']['oneOf'][1]['properties']['le']["properties"]['s']['oneOf'][1]['const'] == __le()[
        coring.Saids.dollar]


def test_oor_auth_chain():
    auth = json.load(open(f'{__path()}/../schema/acdc/v1.1.0/oor-authorization-vlei-credential.json', 'r'))
...
def test_oor_chain():
    oor = json.load(
        open(f'{__path()}/../schema/acdc/v1.1.0/legal-entity-official-organizational-role-vLEI-credential.json',
             'r'))
    auth = json.load(open(f'{Path(__file__).parent}/../schema/acdc/v1.1.0/oor-authorization-vlei-credential.json', 'r'))

    assert oor['properties']['e']['oneOf'][1]['properties']['auth']["properties"]['s']['oneOf'][1]['const'] == auth[
        coring.Saids.dollar]
...
def test_ecr_chain():
    auth = json.load(open(f'{__path()}/../schema/acdc/v1.1.0/ecr-authorization-vlei-credential.json', 'r'))
    ecr = json.load(
        open(f'{__path()}/../schema/acdc/v1.1.0/legal-entity-engagement-context-role-vLEI-credential.json', 'r'))

    assert ecr['properties']['e']['oneOf'][1]['properties']['auth']["properties"]['s']['oneOf'][1]['const'] == auth[
        coring.Saids.dollar]
    assert ecr['properties']['e']['oneOf'][2]['properties']['le']["properties"]['s']['oneOf'][1]['const'] == __le()[
        coring.Saids.dollar]


def __le():
    return json.load(open(f'{__path()}/../schema/acdc/v1.1.0/legal-entity-vLEI-credential.json', 'r'))
